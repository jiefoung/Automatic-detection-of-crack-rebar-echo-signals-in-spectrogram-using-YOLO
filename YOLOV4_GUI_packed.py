...
class DarknetRunner(QThread):
    ...
    def run(self):
        ...
        try:
            # 預測輸出資料夾
            pred_dir = os.path.join(self.run_dir, 'predictions')
            ensure_dir(pred_dir)
            # darknet test 模式參數
            out_json_path = os.path.join(self.run_dir, 'out.json')
            base_args = [
        ...
        # 必須要 test list
            if self.test_list and os.path.isfile(self.test_list):
                ...
                if not images:
                    ...
                    self.done_sig.emit(False, self.run_dir)
                    return
            ...
            # ------------------------------
            # ⭐ 轉換 out.json → YOLO txt
            # ------------------------------
                if ok:
                    try:
                        save_txt_from_outjson(
                        ...
                        self._write_log(f"⚠ 由 out.json 產生 txt 時發生錯誤：{e}")
...
def save_txt_from_outjson(out_json: str, pred_dir: str, data_path: str, log=None):
    """
    將 darknet -out out.json 的結果轉成每張圖片一個 YOLO 預測 txt
    格式：class conf cx cy w h（皆為相對座標）
    """
    # 先從 .data 找到 names 檔，取得類別順序
    data_info = parse_data_file(data_path)
    names_path = data_info.get('names') or data_info.get('names_path')
    if names_path and not os.path.isabs(names_path):
        names_path = os.path.join(os.path.dirname(data_path), names_path)
    _, names_list = count_names(names_path)
    name_to_id = {n.strip(): i for i, n in enumerate(names_list)}

    if log:
        log(f"嘗試從 out.json 轉出預測 txt，names 檔：{names_path}")

    if not os.path.isfile(out_json):
        if log:
            log(f"⚠ 找不到 out.json：{out_json}")
        return

    try:
        with open(out_json, "r", encoding="utf-8") as f:
            raw = f.read()
        # 這一步是關鍵：把 Windows 反斜線路徑換成正斜線，變成合法 JSON
        raw = raw.replace("\\", "/")
        jd = json.loads(raw)
    except Exception as e:
        if log:
            log(f"⚠ 讀取 out.json 失敗：{e}")
        return


    # out.json 可能是 list 或 {results: [...]}
    if isinstance(jd, dict) and "results" in jd:
        records = jd["results"]
    ...
    for rec in records:
        filename = rec.get("filename") or rec.get("name") or rec.get("file_name")
        if not filename:
            continue
        base = os.path.splitext(os.path.basename(filename))[0]
        objects = rec.get("objects") or rec.get("detections") or []

        lines = []
        for obj in objects:
            cls_name = obj.get("name") or obj.get("label") or str(obj.get("class_id"))
            conf = float(obj.get("confidence", obj.get("probability", 0.0)))

            rel = obj.get("relative_coordinates") or obj.get("bbox") or {}
            cx = float(rel.get("center_x", 0.0))
            cy = float(rel.get("center_y", 0.0))
            w  = float(rel.get("width", 0.0))
            h  = float(rel.get("height", 0.0))

            cid = obj.get("class_id")
            if cid is None:
                cid = name_to_id.get(cls_name, -1)
            if cid is None or cid < 0:
                continue

            # 跟單張模式一樣：class conf cx cy w h
            lines.append(f"{cid} {conf:.6f} {cx:.6f} {cy:.6f} {w:.6f} {h:.6f}")

        if lines:
            out_txt = os.path.join(pred_dir, base + ".txt")
            with open(out_txt, "w", encoding="utf-8") as f:
                f.write("\n".join(lines))
            n_files += 1

    if log:
        log(f"已從 out.json 轉出 {n_files} 個預測 txt 到資料夾：{pred_dir}")
